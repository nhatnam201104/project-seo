...
/**
 * Đọc Bearer token, xác minh và nạp danh tính vào
 * {@link SecurityContextHolder}.
 * Token sai/hết hạn → bỏ qua (giữ context rỗng); quyết định 401 để cho entry
 * point xử lý.
 */
@Component
  ...
  private void authenticate(String token) {
    try {
      ...
      var user = userRepository
          ...
          .filter(
              candidate -> candidate.isActive() && candidate.getRole().name().equals(claims.role()))
          .orElseThrow();
